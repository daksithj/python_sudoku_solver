...
class Sudoku:
    ...
    def __init__(self, puzzle, puzzle_size):
        ...
        self.allowed_values = [[[True for k in range(puzzle_size)] for j in range(puzzle_size)]
                               for i in range(puzzle_size)]

        # Array storing the filled values in each row
        self.row_fillings = [[True for k in range(puzzle_size)] for j in range(puzzle_size)]

        # Array storing the filled values in each column
        self.column_fillings = [[True for k in range(puzzle_size)] for j in range(puzzle_size)]

        # Array storing the filled values in each block
        self.box_fillings = [[[True for k in range(puzzle_size)] for j in range(self.box_dim)]
...
def sudoku_solver(file):
    ...
    # Load puzzle from the text file
    puzzle = np.loadtxt(file, dtype=np.int)

    puzzle_size = 0

    # Get puzzle size and exit if invalid shape
    if puzzle.shape[0] == 9 and puzzle.shape[1] == 9:
        puzzle_size = 9
    elif puzzle.shape[0] == 16 and puzzle.shape[1] == 16:
        puzzle_size = 16
    else:
        print("Invalid puzzle shape!")
        exit(1)

    # Create Sudoku object
    sudoku_puzzle = Sudoku(puzzle, puzzle_size)
